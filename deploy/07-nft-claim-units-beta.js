...
const {
  ...
  RONIN_SIGNER,
  SAIGON_SIGNER,
  UNITS_BETA_CONTRACT_ADDRESS,
  SAIGON_UNITS_BETA_CONTRACT_ADDRESS,
} = process.env

const func = async function ({ getChainId, getNamedAccounts, deployments: { deploy } }) {
  const contractName = 'Wild-Forest-Claim-Units-Beta'
  ...
  let nftContractAddress = UNITS_BETA_CONTRACT_ADDRESS

  if (isSaigon) {
    unitsOwnerAddress = SAIGON_UNITS_OWNER_ADDRESS
    signerAddress = SAIGON_SIGNER
    nftContractAddress = SAIGON_UNITS_BETA_CONTRACT_ADDRESS
  }
